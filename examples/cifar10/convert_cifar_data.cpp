...
void convert_dataset(const string& input_folder, const string& output_folder) {
	...
	if (db_backend == "leveldb") { // leveldb
		...
		status = leveldb::DB::Open(options,
				output_folder + "/cifar10_train_leveldb", &train_db);
		CHECK(status.ok()) << "Failed to open leveldb.";
	} else if (db_backend == "lmdb") {  // lmdb
		string train_db_path = output_folder + "/cifar10_train_lmdb";
		CHECK_EQ(mkdir(train_db_path.c_str(), 0744), 0) << "mkdir "
				<< train_db_path << "failed";
		CHECK_EQ(mdb_env_create(&mdb_env), MDB_SUCCESS)
				<< "mdb_env_create failed";
		CHECK_EQ(mdb_env_set_mapsize(mdb_env, 1099511627776), MDB_SUCCESS) // 1TB
		<< "mdb_env_set_mapsize failed";
		CHECK_EQ(mdb_env_open(mdb_env, train_db_path.c_str(), 0, 0664),
	...
	for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
		...
		snprintf(str_buffer, kCIFARImageNBytes, "/data_batch_%d.bin",
				fileid + 1);
		...
		for (int itemid = 0; itemid < kCIFARBatchSize; ++itemid) {
			...
			snprintf(str_buffer, kCIFARImageNBytes, "%05d",
					fileid * kCIFARBatchSize + arr[itemid]);
			...
			if (db_backend == "leveldb") {  // leveldb
				...
				CHECK_EQ(mdb_put(mdb_txn, mdb_dbi, &mdb_key, &mdb_data, 0),
						MDB_SUCCESS) << "mdb_put failed";
			} else {
				LOG(FATAL) << "Unknown db backend " << db_backend;
			}
		}
	...
	if (db_backend == "leveldb") { // leveldb
		CHECK(
				leveldb::DB::Open(options,
						output_folder + "/cifar10_test_leveldb", &test_db).ok())
				<< "Failed to open leveldb.";
	} else if (db_backend == "lmdb") {  // lmdb
		string test_db_path = output_folder + "/cifar10_test_lmdb";
		CHECK_EQ(mkdir(test_db_path.c_str(), 0744), 0) << "mkdir "
				<< test_db_path << "failed";
		CHECK_EQ(mdb_env_create(&mdb_env), MDB_SUCCESS)
				<< "mdb_env_create failed";
		CHECK_EQ(mdb_env_set_mapsize(mdb_env, 1099511627776), MDB_SUCCESS) // 1TB
		<< "mdb_env_set_mapsize failed";
		CHECK_EQ(mdb_env_open(mdb_env, test_db_path.c_str(), 0, 0664),
				MDB_SUCCESS) << "mdb_env_open failed";
		CHECK_EQ(mdb_txn_begin(mdb_env, NULL, 0, &mdb_txn), MDB_SUCCESS)
				<< "mdb_txn_begin failed";
		CHECK_EQ(mdb_open(mdb_txn, NULL, 0, &mdb_dbi), MDB_SUCCESS)
	...
	for (int itemid = 0; itemid < kCIFARBatchSize; ++itemid) {
		...
		datum.SerializeToString(&value);
		//itemid
		snprintf(str_buffer, kCIFARImageNBytes, "%05d", arr[itemid]);
